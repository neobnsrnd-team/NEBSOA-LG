 ...
 * *** empty log message ***
 *
 * Revision 1.5  2007/05/10 01:02:07  오재훈
 * data가 null이면 InvalidException 발생
 *
...
public class LengthValidator {
	...
	public static void validate(int size, String data) throws InvalidException {
		if (!StringUtil.isNull(data)) {
			
			int dataSize = data.length();
			if(dataSize>size) throw new InvalidException(data);
	...
	public static void validate(int size, String key, String data) throws InvalidException {
		if (!StringUtil.isNull(data)) {
			
			int dataSize = data.length();
			if(dataSize>size) throw new InvalidException(key, data);
