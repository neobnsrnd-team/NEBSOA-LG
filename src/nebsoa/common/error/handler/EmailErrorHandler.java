 ...
 * *** empty log message ***
 *
 * Revision 1.4  2007/09/06 11:16:38  최수종
 * *** empty log message ***
 *
 * Revision 1.3  2007/07/19 06:50:19  최수종
 * *** empty log message ***
 *
 * Revision 1.2  2007/06/20 10:42:40  최수종
 * *** empty log message ***
 *
 ...
 * </pre>
 ******************************************************************/
public class EmailErrorHandler extends ErrorHandler {
	...
	public void executeHandler(String userParamVaule, String sysParamValue) throws Exception
	{
		...
		LogManager.debug("##### EmailErrorHandler.executeHandler() 실행 !!");
...
//			System.out.println("{all}sysTemp=>"+sysTemp);
			...
			{		
				...
				{
					if(sysKey.equals("senderEmail")) {  // 보낸사람 이메일주소
						senderEmail = sysTempSt.nextToken().trim();
						sysCount++;					
					}
					else if(sysKey.equals("senderName")) {  // 보낸사람명
						senderName = sysTempSt.nextToken().trim();
					}
					else if(sysKey.equals("ip")) {  // 메일서버 IP
						ip = sysTempSt.nextToken().trim();
						sysCount++;					
					}
					else if(sysKey.equals("port")) {  // 메일서버 port
						port = sysTempSt.nextToken().trim();
						sysCount++;					
					}
					else if(sysKey.equals("id")) {  // 메일서버 ID
						id = sysTempSt.nextToken().trim();
						sysCount++;					
					}
					else if(sysKey.equals("password")) {  // 메일서버 비밀번호
...
//					System.out.println("{all}userValue=>"+userValue);
					...
					if(userKey.equals("receiverEmail")) {  // 받는사람 이메일주소
...
//							System.out.println("userValue=>"+str);
						...
						mailInfo.setReceiverEmail(arrayList);
						userCount++;
					}
					else if(userKey.equals("receiverName")) {  // 받는사람명
...
//							System.out.println("[receiverName]userValue=>"+str);
						...
						mailInfo.setReceiverName(arrayList);
					}		
					else if(userKey.equals("subject")) {  // 메일 제목
						
						mailInfo.setSubject(userValue);
						userCount++;
					}	
					else if(userKey.equals("content")) {  // 메일 내용

						mailInfo.setContent(userValue);
						userCount++;
					}	
					else
					{
...
//		System.out.println("userCount=>"+userCount);
		...
		// 필수 입력항목 갯수 체크
		if(sysCount >= 5 && userCount >= 3) {
			
			EmailManager.send(ip, port, id, password, mailInfo);
		}
		else
		{
			// 메일 보내지 않음
...
//			"receiverEmail=afoc771@nate.com,ohmyallaha@nate.com;receiverName=홍길동,김길동;subject=멜테스트;content=테스트입니다.";
			"receiverEmail=afoc771@nate.com,ohmyallaha@nate.com;receiverName=;subject=멜테스트1881112345;content=테스트입니다.112322245";			
		String sysParamValue = 
//			"ip=211.115.70.148;port=25;id=nonorain11;password=111;senderEmail=ohmyallah@nate.com;senderName=아무개";
			"ip=211.115.70.148;port=25;id=nonorain11;password=111;senderEmail=ohmyallah@nate.com;senderName=";
		String executeResultText = null;
		String executeResultCode = null;
		try
		{
		
			EmailErrorHandler handler = new EmailErrorHandler();
