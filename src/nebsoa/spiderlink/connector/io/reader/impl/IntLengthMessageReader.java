 ...
 * <pre>
 * 1.설명 
 * 최초 4byte를 읽어  전문 길이를 파악한 후 
 * 해당 길이만큼  읽어  데이터를 BYTE[]로 리턴한다.
 * 앞에 읽은 4byte는 합쳐 지지 않는다.
 * 2.사용법
 * IntLengthDataReader reader = IntLengthDataReader();
 ...
 * - 변경이력 (버전/변경일시/작성자)
 * 
 * $Log: IntLengthMessageReader.java,v $
 ...
 * </pre>
 ******************************************************************/
public class IntLengthMessageReader extends ShortLengthMessageReader {

    public IntLengthMessageReader(DataInputStream in, String log){
        super(in,log);
        setLengthPartCounted(false);
    }
    
    public IntLengthMessageReader(DataInputStream in){
        this(in, "DEBUG");
        setLengthPartCounted(false);
    }
    
    public IntLengthMessageReader(){
        super();
        setLengthPartCounted(false);
    }
