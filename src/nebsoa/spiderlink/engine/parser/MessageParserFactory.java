 ...
 * <pre>
 * 1.설명 
 * 메시지 타입에 따라 Message parser를 생성해주는 Factory 클래스
 * 2.사용법
 * 
 * <font color="red">
 * 3.주의사항
 ...
 * - 변경이력 (버전/변경일시/작성자)
 * 
 * $Log: MessageParserFactory.java,v $
...
public class MessageParserFactory {
	...
	private static final String POSTFIX = "_Parser";
	
	public static MessageParser getMessageParser(MessageType messageType) {
		if(messageType.getType().equals(MessageType.FIXED_LENGTH_STRING)){
    		return FixedLengthMessageParser.getInstance();
    	}else if(messageType.getType().equals(MessageType.XML_STRING)){
    		return XMLMessageParser.getInstance();
    	}else if(messageType.getType().equals(MessageType.MAP_STRING)){
    		return DelimitedMapParser.getInstance();
    	}else if(messageType.getType().equals(MessageType.CSV_STRING)){
    		return CsvMessageParser.getInstance();
    	}else{
    		
    		String messageParser = PropertyManager.getProperty("spiderlink", messageType.getType()+POSTFIX, null);
    		if(StringUtil.isNull(messageParser)){
    			throw new SysException("지원되지 않는 Message Type입니다. ["+messageType.getType()+"]");
    		}else{
    			try{
	    			Class messageGeneratorClass = Class.forName(messageParser);
	    			Method method = messageGeneratorClass.getMethod("getInstance", null);
	    			return (MessageParser)method.invoke(messageGeneratorClass, null);
    			}catch(Exception e){
    				throw new SysException("MessageParser 생성에 실패했습니다.["+messageType.getType()+"], 원인:" + e);
    			}
