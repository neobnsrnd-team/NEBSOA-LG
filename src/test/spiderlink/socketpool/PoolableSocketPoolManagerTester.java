 ...
 */
public class PoolableSocketPoolManagerTester {
    static int i=0;
    int count;
    ...
    public PoolableSocketPoolManagerTester(String ip, int port) {
       this(ip,port,10);
    }
    
   public PoolableSocketPoolManagerTester(String ip, int port, int timeout) {
       this.count=i++;
       pool = SocketPoolManager.getInstance().getPool(ip+"_"+port);
       if(pool==null){
           SocketFactory factory = new PoolableSocketFactory(ip,port,timeout,"DEBUG");
           pool=SocketPoolManager.getInstance().makeSocketPool(
                   ip+"_"+port,factory,1,1,true);
       }
   }
...
//        }
        ...
        //System.out.println(testStr+" write and  ok");
   ...
   public static void main(String[] args) {
      LogManager.debug("\n\n********* Starting Test ************************");
      for(int i=0;i<10;i++){
          PoolableSocketPoolManagerTester pt = 
              new PoolableSocketPoolManagerTester("127.0.0.1",10001);    
...
//      for(int i=0;i<1;i++){
//          AsyncSocketPoolManagerTester pt = 
//              new AsyncSocketPoolManagerTester("127.0.0.1",10001);    
//          pt.run(); // test using wait() and notify()
//          //if(i%10==1){
//              SocketPoolManager.getInstance().logSocketPoolState();
//          //}
...
//      }
      try {
          System.out.println("sleep 200 second...");
