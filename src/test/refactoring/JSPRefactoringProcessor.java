 ...
 * - 변경이력 (버전/변경일시/작성자)
 * 
 * $Log: JSPRefactoringProcessor.java,v $
 ...
 * *** empty log message ***
 *
 * Revision 1.1  2007/01/29 09:08:27  이종원
 * *** empty log message ***
 *
...
public class JSPRefactoringProcessor {

    public static void main(String[] args){
        File src = new File("/refactoring/src/admin");
        ...
        for(int i=0;i<files.length;i++){
            if(files[i].isDirectory()){
                ...
                if(files[i].getName().endsWith("jsp")){
                    updateFile(src,files[i].getName(),target,"XecureSubmit(form1)","form1.submit()");
                }
            }
        }
    }
