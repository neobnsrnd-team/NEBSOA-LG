 ...
 * - 변경이력 (버전/변경일시/작성자)
 * 
 * $Log: RefactoringProcessor2.java,v $
...
public class RefactoringProcessor2 {
    ...
    public static void updateDir(File src, File target){
        ...
        if(!target.exists() && !target.getName().endsWith("action")) {
            target.mkdir();
        ...
        for(int i=0;i<files.length;i++){
            if(files[i].isDirectory()){
                ...
                if(files[i].getName().endsWith(".java")){
                    if(target.getName().endsWith("action")){
                        target = target.getParentFile();
                    }
                    updateFile(src,files[i].getName(),target,".action;",";");
                }
            }
        }
    }
