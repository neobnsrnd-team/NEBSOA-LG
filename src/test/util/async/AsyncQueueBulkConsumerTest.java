...
/**
 * @author 이종원
 * Tester class for AsyncQueueBulkConsumerTest
 */
public class AsyncQueueBulkConsumerTest {
	...
	public static void main(String[] args) {
		...
		if(config == null) {
			int queueCount=1;  
			int queueSize=1000;   //bulk test를 위하여 size 1000 으로 조정.
			...
			config = AsyncQueueManager.getInstance().
      		makeAsyncQueue("test", queueCount,queueSize,procuderWaitTimeout,
      				consumerWaitTimeout,consumerSleepTime,"nebsoa.util.async.Consumer",true);
		}
		
		for(int i=0;i<100;i++){
			Thread t4 = new TestProducer4();
			t4.start();
			Thread t5 = new TestProducer5();
			t5.start();
		}
		try {
	...
	public static class TestProducer4 extends Thread {
		public void run(){
			QueueConfig config = AsyncQueueManager.getInstance().getQueueConfig("test");
			for(int i=0;i<50;i++){
				try {
	...
	public static class TestProducer5 extends Thread {
		public void run(){
			QueueConfig config = AsyncQueueManager.getInstance().getQueueConfig("test");
			for(int i=0;i<50;i++){
				try {
